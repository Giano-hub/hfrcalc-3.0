...
$(".move-selector").change(function () {
	...
	if (m) {
		...
		if (actual.type !== m[1] && !["Calculator"].includes(game)) {
         ...
         $(".tag-container").hide();
            ...
            for (var i in flags["battleType"]["triple"]) {
					if (flags["battleType"]["triple"][i].includes(window.CURRENT_TRAINER)) {
						...
						trainerHTML = "";
						var nextTrainerPokemon = getTrainerPokemon(` (${triple[0]})`);
						for (var i in nextTrainerPokemon) {
							if (nextTrainerPokemon[i][0].includes($('input.opposing').val())) {
								continue;
							}
...
function correctHiddenPower(pokemon) {
	if (["Calculator"].includes(game)) return pokemon;
...
function loadDefaultLists() {
	...
	if (element.hasClass("opposing") && game !== "None") {
		...
		if (saved && partyOrder && saved in partyOrder) {
			var party = partyOrder[saved];
			var firstMon = party[0];
			var total = party.filter(function (s) { return s === firstMon; }).length;
			var savedKey = total > 1 ? saved + " (1)" : saved;
			var match = getSetOptions().find(function (o) {
				return o.set === savedKey && o.pokemon === firstMon;
			});
			if (match) {
				callback(match);
				return;
...
function updateTopSprite(fullSetName, imgId) {
    ...
    if (cleanName === "Castform") {
        ...
        // Troviamo il pannello (#p1 per p1mon, #p2 per p2mon)
        var pId = (imgId === "p1mon") ? "p1" : "p2";
        
        var $type1 = $('#' + pId + ' .type1');
        var $type2 = $('#' + pId + ' .type2');

        // Fallback nel caso l'HTML non usi gli ID #p1/#p2
        if (!$type1.length) {
            var selector = (imgId === "p1mon") ? '.player' : '.opposing';
            $type1 = $(selector).closest('div, table, form, fieldset').find('.type1');
            $type2 = $(selector).closest('div, table, form, fieldset').find('.type2');
        }

        // Impostiamo il nuovo tipo e aggiorniamo il calcolatore
        if ($type1.length) {
            $type1.val(newType).trigger('change');
            $type2.val("").trigger('change');
        }
    }
    // ----------------------------------------

    // Convertiamo il nome per lo sprite locale (es: "Castform-Sunny" -> "castformsunny")
...
function maybeRefreshColorCode() {
	if (document.getElementById("cc-auto-refr").checked && $("#show-cc")[0].hidden) {
...
$(document).ready(function () {
   ...
   if (typeof partyOrder !== "undefined" && localStorage.getItem("lastTrainer") in partyOrder) {
	...
	var firstMon = party[0];
	var total = party.filter(function (s) { return s === firstMon; }).length;
	var savedKey = total > 1 ? savedTrainer + " (1)" : savedTrainer;
	$("#p2 .set-selector").val(firstMon + " (" + savedKey + ")");
   }  
   $(".set-selector").change();
   ...
   $('#cc-advanced').change(refreshColorCode);
	...
	$('#bait-help').click(() => {
		alert("This section predicts what will Pokémon will be sent out after your Pokémon defeats each member of the enemy party.\n\n" + 
			  ...
			  (["Calculator"].includes(game) ? "In very rare cases such as damage overflow, you can use the slower but more accurate \"Advanced Bait\" option.\n\n" : "") + 
			  "In case of an incorrect result, please DM me on Discord at @giano8334, or open a GitHub issue.");
	});
	$('#ai-help').click(() => {
