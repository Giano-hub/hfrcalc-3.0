...
$(".move-selector").change(function () {
	...
	if (m) {
		...
		if (actual.type !== m[1] && !["Hellfire Red"].includes(game)) {
...
$(".set-selector").change(function () {
	...
	if ($(this).hasClass('opposing') && game != "None") {
		...
		for (var i in nextPokemon) {
			...
			var pokemonName = nextPokemon[i].split("]")[1].trim().split(" (")[0];
				...
				for (var i in flags["battleType"]["tag"]) {
					if (flags["battleType"]["tag"][i].includes(window.CURRENT_TRAINER)) {
						...
						for (var i in nextTrainerPokemon) {
							...
							var pokemonName = nextTrainerPokemon[i].split("]")[1].trim().split(" (")[0];
						...
						for (var i in nextTagPokemon) {
							...
							var pokemonName = nextTagPokemon[i].split("]")[1].trim().split(" (")[0];
...
function correctHiddenPower(pokemon) {
	if (["Hellfire Red"].includes(game)) return pokemon;
...
$(document).ready(function () {
	...
	$('#bait-help').click(() => {
		alert("This section predicts what will Pokémon will be sent out after your Pokémon defeats each member of the enemy party.\n\n" + 
			  ...
			  (["Hellfire Red"].includes(game) ? "In very rare cases such as damage overflow, you can use the slower but more accurate \"Advanced Bait\" option.\n\n" : "") + 
